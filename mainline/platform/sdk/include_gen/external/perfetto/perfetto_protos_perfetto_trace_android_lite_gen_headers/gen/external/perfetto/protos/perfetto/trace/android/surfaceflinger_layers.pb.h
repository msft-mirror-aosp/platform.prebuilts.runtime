...
constexpr HwcCompositionType HwcCompositionType_MAX = HWC_TYPE_SIDEBAND;
...
class LayersTraceFileProto final :
 ...
 public:
  ...
  LayersTraceFileProto* New() const {
    return New(nullptr);
  }
...
class LayersSnapshotProto final :
 ...
 public:
  ...
  inline void Swap(LayersSnapshotProto* other) {
    if (other == this) return;
  #ifdef PROTOBUF_FORCE_COPY_IN_SWAP
    if (GetOwningArena() != nullptr &&
        GetOwningArena() == other->GetOwningArena()) {
  ...
  LayersSnapshotProto* New(::PROTOBUF_NAMESPACE_ID::Arena* arena) const final {
    return CreateMaybeMessage<LayersSnapshotProto>(arena);
  }
  LayersSnapshotProto* New() const {
    return New(nullptr);
  }
  void CheckTypeAndMergeFrom(const ::PROTOBUF_NAMESPACE_ID::MessageLite& from)  final;
